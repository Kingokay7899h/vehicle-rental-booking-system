...
const App = () => {
  ...
  const handleSubmit = async () => {
    if (validateStep(4)) {
      ...
      try {
        ...
        setIsLoading(false);
        if (response.ok) {
          // Using alert is okay for this version as requested, but a custom modal is better
  ...
  // --- UI COMPONENTS (UBER STYLE & RESPONSIVE) ---
  const steps = [
    {
    ...
    },
    {
      ...
      component: () => (
        <div className="w-full max-w-lg space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { value: '4', label: '4 Wheels', icon: '🚗', desc: 'Cars & SUVs' },
              { value: '2', label: '2 Wheels', icon: '🏍️', desc: 'Motorcycles' }
            ].map((option) => (
              <div
                key={option.value}
                onClick={() => setFormData({...formData, wheels: option.value, vehicleType: '', specificModel: ''})}
                className={`p-6 rounded-lg border-2 text-center cursor-pointer transition-all duration-300 transform-gpu group ${
    ...
    },
    {
      ...
      component: () => (
        <div className="space-y-6 w-full max-w-4xl">
          {isLoading ? (
            <div className="flex justify-center items-center h-48"><CircularProgress size={40} sx={{ color: '#000000' }} /></div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-h-[450px] overflow-y-auto p-2">
              {vehicles.map((model) => (
                <div
                  ...
                  className={`rounded-lg border-2 cursor-pointer transition-all duration-300 group overflow-hidden ${
                ...
                >
                  <div className="bg-gray-100 overflow-hidden aspect-video">
                    <img src={VEHICLE_IMAGES[model.name]} alt={model.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-in-out"/>
                  </div>
                  <div className="p-4 flex justify-between items-center">
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                      formData.specificModel === model.id ? 'border-black bg-black' : 'border-gray-300 bg-white'
                    }`}>
          ...
          <div className="space-y-6 w-full max-w-xl">
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2, alignItems: 'center', justifyContent: 'center' }}>
              <DatePicker
                ...
                minDate={dayjs()}
                slotProps={{ textField: { error: !!errors.startDate, helperText: errors.startDate, fullWidth: true, sx: { '& .MuiOutlinedInput-root': { borderRadius: '8px', backgroundColor: '#f3f4f6' } } } }}
              />
              ...
              <DatePicker
                ...
                slotProps={{ textField: { error: !!errors.endDate, helperText: errors.endDate, fullWidth: true, sx: { '& .MuiOutlinedInput-root': { borderRadius: '8px', backgroundColor: '#f3f4f6' } } } }}
              />
            </Box>
              ...
              <Box sx={{ textAlign: 'center', mt: 2 }}>
                <Paper elevation={0} sx={{ p: 2, borderRadius: '8px', backgroundColor: '#f3f4f6' }}>
                  <Typography variant="body1" color="text.secondary">
                    Total Duration: <span className="font-bold text-black">{formData.endDateObj.diff(formData.startDateObj, 'day')} day(s)</span>
                  </Typography>
                </Paper>
              </Box>
            )}
  ...
  const ConfirmationStep = () => (
    <div className="text-center space-y-4 w-full max-w-2xl">
        <h2 className="text-3xl font-bold text-black mb-2">Review Your Booking</h2>
        <p className="text-gray-500 text-lg">Please confirm the details below.</p>
        <Paper 
            elevation={0} 
            sx={{ p: { xs: 2, sm: 4 }, borderRadius: '12px', backgroundColor: '#ffffff', border: '1px solid #e5e7eb', textAlign: 'left' }}
        >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-gray-700">
                ...
                <div><p className="text-xs uppercase tracking-wider font-medium text-gray-500">Vehicle</p><p className="font-semibold text-black">{getVehicleName(formData.specificModel)}</p></div>
                ...
                <div className="sm:col-span-2 mt-2 pt-3 border-t"><p className="text-center text-lg">Total Duration: <span className="font-bold text-black">{dayjs(formData.endDate).diff(dayjs(formData.startDate), 'day')} days</span></p></div>
            </div>
        </Paper>
    </div>
  );
  ...
  const SuccessStep = () => (
    <div className="text-center space-y-4 py-8">
        ...
        <h2 className="text-3xl font-bold text-black">Booking Confirmed!</h2>
        <p className="text-gray-500 text-lg max-w-md mx-auto">Thank you, {formData.firstName}! Your booking for the {getVehicleName(formData.specificModel)} is complete.</p>
    </div>
  );
  
  const allSteps = [...steps, 
    { title: "Confirm", subtitle: "One last check before we go.", icon: <Check className="w-8 h-8" />, component: ConfirmationStep },
                    ...
                    <React.Fragment key={index}>
                        <div className="flex flex-col items-center z-10">
                            <div
                                ...
                                index < currentStep ? 'bg-green-500 text-white' : index === currentStep ? 'bg-black text-white scale-110' : 'bg-gray-200 text-gray-500'
                                }`}
                            >
                        ...
                        {index < 5 && (
                            <div className={`flex-auto h-1 -mx-1 transition-all duration-500 ${index < currentStep ? 'bg-black' : 'bg-gray-200'}`}></div>
                        )}
                    </React.Fragment>
                ))}
            </div>
          ...
          <div className={`transition-all duration-300 ${isAnimating ? 'opacity-0 transform translate-x-4' : 'opacity-100 transform translate-x-0'}`}>
            ...
            {currentStep < allSteps.length - 1 && (
                 ...
                 <Button
                   ...
                   variant="text"
                   sx={{ 
                     textTransform: 'none', fontSize: '16px', fontWeight: 600, color: '#000000', 
                 ...
                 {currentStep < allSteps.length - 2 ? (
                   <Button
                     ...
                     variant="contained"
                     sx={{ 
                         textTransform: 'none', fontSize: '16px', fontWeight: 600, color: 'white',
                         backgroundColor: '#000000', borderRadius: '8px', 
                         padding: '12px 32px', boxShadow: 'none',
